...
export default function AdminGymPageSection() {
  ...
  const renameBlock = async (id, newLabel) => {
    ...
    await supabase.from('gym_page_blocks').update({ label: newLabel.trim() }).eq('id', id)
    load()
  }
  const handleReorder = async (newBlocks) => {
    const updates = newBlocks.map((b, i) => ({ id: b.id, ordre: (i + 1) * 10 }))
    await Promise.all(updates.map(u => supabase.from('gym_page_blocks').update({ ordre: u.ordre }).eq('id', u.id)))
    load()
  }
  ...
  const addBlock = async (key, customLabel) => {
    ...
    await supabase.from('gym_page_blocks').insert({ block_key: key, label: customLabel || (genericType(key) ? (GENERIC_BLOCKS.find(g => g.type === genericType(key))?.label ?? key) : (BLOCK_META[key]?.label ?? key)), visible: true, content: {}, ordre: maxOrdre + 10 })
