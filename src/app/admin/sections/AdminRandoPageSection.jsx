...
export default function AdminRandoPageSection() {
  ...
  const handleReorder = async (newBlocks) => {
    ...
    await Promise.all(updates.map(u => supabase.from('rando_page_blocks').update({ ordre: u.ordre }).eq('id', u.id)))
    load()
  }
  ...
  const addBlock = async (key, customLabel) => {
    ...
    await supabase.from('rando_page_blocks').insert({ block_key: key, label: customLabel || (genericType(key) ? (GENERIC_BLOCKS.find(g => g.type === genericType(key))?.label ?? key) : (BLOCK_META[key]?.label ?? key)), visible: true, content: {}, ordre: maxOrdre + 10 })
