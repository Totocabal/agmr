...
export default async function AssoPage() {
  ...
  const histoire   = c('histoire')
  const sections   = c('sections')
  const gouv       = c('gouvernance')
  const affiliations = Array.isArray(c('affiliations').cards) ? c('affiliations').cards : DEFAULT_AFFILIATIONS
  const docsAsso   = Array.isArray(c('documents_asso').docs)  ? c('documents_asso').docs  : DEFAULT_DOCS
        ...
        {vis('header') && (
          <div className="page-header">
            <div className="container">
              ...
              <div className="page-header-eyebrow">{c('header').eyebrow ?? "L'association · Loi 1901 · Fondée en 1970"}</div>
              <h1>{c('header').titre ?? 'Une association, un esprit'}</h1>
              <p className="page-header-lede">{c('header').lede ?? "L'Association Gym Marche Rambouillet est le nouveau nom du Club Loisirs et Détente. Renommée en novembre 2018, elle compte aujourd'hui environ 750 adhérents."}</p>
            </div>
          </div>
        )}
                ...
                {vis('histoire') && (
                  <>
                    ...
                    <p style={{ color: "var(--ink-soft)" }}>
                      {histoire.texte1 ?? "L'Association Gym Marche Rambouillet (AGMR) est le nouveau nom du Club Loisirs et Détente (CLD). C'est une association selon la loi de 1901, créée en 1970. Le changement de nom a été voté lors de l'assemblée générale extraordinaire de novembre 2018."}
                    </p>
                    <div style={{ margin: "20px 0", padding: "20px 24px", borderLeft: "3px solid var(--accent)", background: "var(--accent-tint)", borderRadius: "0 var(--r-sm) var(--r-sm) 0" }}>
                      <p style={{ fontFamily: "var(--serif)", fontStyle: "italic", fontSize: "1.15rem", color: "var(--accent-deep)", margin: 0 }}>
                        « {histoire.citation ?? "Ces activités sont pratiquées dans un esprit de détente et de convivialité favorisant l'épanouissement de chacun, en dehors de toute notion de compétition."} »
                      </p>
                    </div>
                    {(histoire.texte2 || !histoire.titre) && (
                      <p style={{ color: "var(--ink-soft)" }}>
                        {histoire.texte2 ?? "Ouverte à tous les courants de pensées, l'association s'interdit toutes discussions confessionnelles ou politiques."}
                      </p>
                    )}
                  </>
                )}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 8 }}>
                      <div style={{ padding: 22, background: "var(--bg-card)", border: "1px solid var(--line)", borderRadius: "var(--r-md)" }}>
                        <div style={{ fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--accent)", marginBottom: 8 }}>
                          {sections.gym_label ?? 'Section Gym'}
                        </div>
                        <div style={{ fontFamily: "var(--serif)", fontSize: "1.6rem", fontWeight: 500, marginBottom: 8 }}>
                          {sections.gym_chiffre ?? '~460 pratiquants'}
                        </div>
                        <p style={{ margin: 0, fontSize: "0.92rem", color: "var(--ink-soft)" }}>
                          {sections.gym_texte ?? 'Affiliée à la FFEPGV. 41h de cours, 5 salles, 8 animatrices et animateurs diplômés.'}
                        </p>
                      </div>
                      <div style={{ padding: 22, background: "var(--bg-card)", border: "1px solid var(--line)", borderRadius: "var(--r-md)" }}>
                        <div style={{ fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--accent)", marginBottom: 8 }}>
                          {sections.marche_label ?? 'Section Marche'}
                        </div>
                        <div style={{ fontFamily: "var(--serif)", fontSize: "1.6rem", fontWeight: 500, marginBottom: 8 }}>
                          {sections.marche_chiffre ?? '~290 pratiquants'}
                        </div>
                        <p style={{ margin: 0, fontSize: "0.92rem", color: "var(--ink-soft)" }}>
                          {sections.marche_texte ?? 'Affiliée à la FFR. Sorties jeudi, dimanche, marche nordique mardi/samedi, séjours. 25 animateurs bénévoles.'}
                        </p>
                      </div>
                    </div>
                  </>
                ...
                {vis('gouvernance') && (
                  <>
                    <h2 id="gouvernance" style={{ fontSize: "1.8rem", marginTop: 48, marginBottom: 8 }}>
                      {gouv.titre ?? 'Gouvernance'}
                    </h2>
                    <p style={{ color: "var(--ink-soft)", marginBottom: 20 }}>
                      {gouv.texte ?? "L'association est dirigée par un comité directeur élu en assemblée générale. Il se réunit 2 fois par trimestre."}
                    </p>
                ...
                {vis('affiliations') && (
                  <>
                    <h2 id="affiliations" style={{ fontSize: "1.8rem", marginTop: 48, marginBottom: 16 }}>
                      {c('affiliations').titre ?? 'Affiliations & labels'}
                    </h2>
                ...
                {vis('documents_asso') && (
                  <>
                    <h2 id="documents" style={{ fontSize: "1.8rem", marginTop: 48, marginBottom: 16 }}>
                      {c('documents_asso').titre ?? 'Documents officiels'}
                    </h2>
